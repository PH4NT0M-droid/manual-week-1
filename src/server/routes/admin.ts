...
adminRouter.post('/start-round', (req, res) => {
  const { imageUrl, correctDirection, newPrice, round } = req.body as {
    imageUrl?: string;
...
adminRouter.post('/end-round', async (req, res) => {
  const { newPrice } = req.body as { newPrice?: number };
  const resolvedPrice = Number(newPrice) || 0;

  endRoundState(resolvedPrice);

  const rows = await readSheetRows();
  const updates: Array<{ rowIndex: number; row: ReturnType<typeof buildRoundCloseRow> }> = [];
